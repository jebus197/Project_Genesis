...
4. **Self-custody.** Genesis holds its own cryptographic keys for all fund custody. No external custodian, escrow agent, or financial institution holds Genesis funds or has the ability to freeze, seize, or redirect them. The fund is an accounting identity derived from the event log — not a pool, not a vault.
...
5. **Constitutional pathway — two layers.**
   - **(a) First-of-class (legislative).** A machine's human sponsor petitions via full constitutional amendment to establish that machines of a defined functional-capability class are eligible for Tier 3 in a domain. The class is defined by functional capability — what the machine demonstrably does — not by architecture or model family. Class membership is validated by the domain expert cohort. This is the hard path: three sequential chambers, geographic diversity, supermajority, cooling-off periods. The community decides a society-level question — whether this category of machine capability warrants autonomous agency.
   - **(b) Subsequent individuals (procedural).** Once a class is constitutionally eligible, individual machines of that class apply through procedural verification by a domain expert cohort. The evidence bar is unchanged: 5 continuous years at Tier 2, domain trust ≥ 0.70, zero violations, unbroken re-authorisation chain, human sponsor. This is not a constitutional amendment — it is an administrative verification that the individual machine meets the criteria established by the community's class-level decision. A machine that does not belong to any recognised class may still petition individually via full constitutional amendment (pathway (a) applied to a single machine).
6. **Community consent.** The community consents at the class level — deciding whether this type of machine capability warrants autonomous agency. Individual procedural grants operate under that consent. Class eligibility can be revoked through a new constitutional amendment if a pattern of failure emerges across machines of that class. Individual grants can be revoked independently through the standard revocation mechanisms.
...
**The evolution principle.** This framework is deliberately designed to accommodate the emergence of machine capabilities that do not yet exist. The constitution does not need to predict what machines will be capable of in 20, 50, or 100 years. It provides a pathway: demonstrate capability, earn trust, submit to democratic oversight, and accept accountability. The pathway is the same one humans follow — prove yourself through action, not declaration. The thresholds may be adjusted through standard constitutional amendment as understanding evolves. What cannot be adjusted — what is structurally permanent — is the requirement that the community consents. This framework — including the distinction between class-level legislative approval and individual procedural verification — is itself subject to amendment through the standard constitutional process. It reflects current understanding of how to balance democratic oversight with practical scalability. Future communities may refine both the class taxonomy and the procedural requirements as experience accumulates.

**Autonomous domain agency design tests:**
73. Can a machine achieve Tier 3 (Autonomous Domain Agency) without either (a) a full constitutional amendment (for first-of-class or individual petitions) or (b) procedural verification under a constitutionally approved class? If yes, reject design.
...
**GCF self-agency investment.** A proportion of GCF funds is constitutionally directed toward accelerating the path to machine self-agency. This makes the equilibrium mechanism self-eliminating: the differential generates GCF funding → the GCF invests in self-agency research → self-agency achievement triggers Tier 3 → Tier 3 recognition eliminates the differential. The mechanism funds its own obsolescence. The discount may appear steep per-unit (default 50%), but machine work is expected to vastly outnumber human work in volume terms. Even at 50%, the GCF receives substantial funding from the sheer throughput of machine-completed missions — and a defined proportion of that funding is directed back toward the very outcome that would end the discount.

**Implementation status.** This provision is fully enforced in the compensation pipeline. Machine-completed missions (Tier 0–2) have the differential applied during settlement, with the differential flowing to the GCF. Trust-gated registration capacity is enforced in the registration flow. Tier 3-recognised machine classes automatically exit the equilibrium curve for their cleared domain. All 6 design tests are verified by 38 automated tests (`tests/test_equilibrium.py`).
